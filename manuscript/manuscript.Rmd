...
The first was human urine spiked with a diverse mixture of `r nrow(spiked_urine_compound_info)` chemical standards from a previously validated assay that included `r spiked_urine_compound_MFs` unique molecular formulas [@beckmann2020].
...
tar_read(assignment_results) %>%
  left_join(tar_read(assignment_matches), 
            by = c("matrix", "technique")) %>% 
  mutate(matrix = paste(matrix,technique)#,
  ) %>%
  mutate(n_assignments = glue::glue(
    '{n_assignments} ({signif(n_assignments/n_features * 100,digits = 3)}\\%)'),
    across(where(is.numeric),~prettyNum(.x,big.mark = ','))) %>%
  select(-technique) %>% 
  dplyr::rename(`\\textit{m/z} features` = n_features,
  ...
  slice(c(1,3,8,2,7,5,4,6)) %>%
  set_colnames(stringr::str_split_fixed(colnames(.),
                                        ' ',2)[,2]) %>% 
...
Supplementary tables S2, S3, S4 and S5 showing adduct, isotopic, transformation and R package benchmarking information respectively.
