...
For instance, if two features are posited to have the same molecular formula based on a mass difference equivalent to ^13^C but are not significantly correlated within the matrix, it would then be illogical to infer biological assumptions about associations (eg. metabolic pathway changes) with other putatively assigned features.

Here, we outline the development of a data-driven, automated approach for 'first pass' putative molecular formula assignment in non-targeted ESI-MS based metabolomics data. 
...
The second was human urine purchased from BioIVT (Catalog# HUMANURINEPNN, Lot# 0393918) and spiked with the sample mixture of chemical standards used above.
...
The sample matrices were all analysed using FIE-HRMS as well as C18 reverse phase ultra-high performance liquid chromatography high-resolution mass spectrometry (C18-UHPLC-HRMS) profiling of the *B. distachyon* leaf tissue.
...
The data were then pre-treated by performing batch correction, feature occupancy filtering, imputation, relative standard deviation (RSD) filtering based on quality control (QC) samples and total ion count (TIC) normalisation using the R package *metabolyseR* v`r packageVersion('metabolyseR')` available from GitHub (https://github.com/jasenfinch/metabolyseR).
...
All components with a size of less than two were removed.
Isotopic *m/z* present without their non-isotopic counterparts are unlikely for molecular formulas below 1000 amu. Therefore, components containing only isotopic features or isotopic adducts were excluded.
...
The input data simply consists of a sample by accurate *m/z* feature intensity matrix with at least four samples.
It is recommended that these data are pre-treated prior to assignment and data preparation can be performed by a software tool of the user's preference.
Pre-treatment steps such as occupancy or QC-based RSD filtering can ensure the robustness of the *m/z* that are considered. 
Other steps such as batch correction, imputation and TIC normalisation can improve the strength of *m/z* correlations by removing variability introduced during MS analysis. 

All assignment parameters are fully customisable, which includes error thresholds, correlation analysis parameters and the specification of custom adducts, isotopes and transformations.
...
However, this is expected as there and are more *m/z* features present with greater numbers of background compounds and therefore a greater chance for alternative graphical components to be present in the correlation network.
...
The assignments obtained from the *B. distachyon* matrices are provided as an example showing the intended real world application of the molecular formula assignment approach to a sample set that includes multiple underlying experimental classes in the correlation network.
...
The C18-UPHLC-HRMS data from *B. distachyon* had the highest percentage of *m/z* features assigned along with the greatest number of molecular formulas. 
...
Added to this, the LC-MS reduces the effects of ion suppression compared to FIE-HRMS and also decreases the chance of potential interference in the correlations between related features due to the presence of structural isomers and isobaric compounds [@gowda2014]. 
...
assignment_results %>%
  ...
  dplyr::rename(
    ...
    `A\\&I iterations assignments` = n_ai_assignments,
    `T iterations assignments` = n_t_assignments,
  ...
  knitr::kable(
    ...
    caption = 'A summary of the molecular formula assignment results for the example sample matrices. The \\textit{m/z} assigned for the adduct and isotopic (A\\&I) iterations and the transformation (T) iterations steps are included.') %>%
  kable_styling(
    font_size = 10,
    latex_options = "HOLD_position"
  ) %>%
...
Although the examples used here include FIE-HRMS and C18-UHPLC-HRMS profiling techniques, other non-targeted metabolomics techniques using ultra-high resolution ESI-MS data such as nanospray direct infusion would also be suitable [@southam2017].
