...
Features matching to possible ionisation products of the chemical standards were identified by first applying the MZedDB ionisation 'rules' and calculating their theoretical *m/z* for each of the adducts specified in Section \@ref(relationships) [@draper2009].
...
compounds_percent_assigned <- standards_compound_outcomes %>% 
  ...
  dplyr::summarise(
    percent = {Assigned / `Matched to m/z features` * 100} %>% 
      signif(digits = 3)
  ) %>% 
  dplyr::pull(percent) %>% 
  purrr::set_names(c(
...
To assess the performance of the molecular formula and graphical component selection approaches, the assignment outcomes were evaluated for the theoretical ionisation products calculated for the chemical standards that matched to *m/z* features in the standards mix and spiked urine. 
...
To simplify interpretation, these evaluations were based on the assignment outcomes of the non-isotopic ionisation products during the adduct and isotopic assignment iterations.
This reduces the incidence of overlap between ionisation products, where multiple theoretical ionisation products match to a single *m/z* feature as well as a removes the inclusion of relationships involving transformations that complicates the calculation and interpretation of relevant relationships.
Based on the `r n_adducts` adduct rules specified, there were `r nrow(standards_assignment_outcomes_non_iso)` and `r nrow(spiked_urine_assignment_outcomes_non_iso)` theoretical ionisation products for these chemical standards that matched to `r n_features$standards` and `r n_features$spiked_urine` *m/z* in the standards mix and spiked urine respectively.
The theoretical ionisation products that directly matched *m/z* features, including the isotopic theoretical ionisation products, are provided in Additional file 5.
The frequencies of assignment outcomes for the theoretical ionisation products in these matrices are shown in Table \@ref(tab:ip-outcomes).
...
Of the `r standards_n_AI_relevant` chemical standards that had theoretical ionisation products with relevant relationships in the standards mix, the molecular formula and graphical component selection approaches were able to correctly assign molecular formulas to `r standards_n_AI_assigned` (`r standards_percent_AI_assigned`%) of these.
For the `r xfun::numbers_to_words(standards_n_alternative)` ionisation products for which an alternative adduct and molecular formula were assigned, two of these were assigned to alternative components with a higher $P_c$ score resulting from a higher component AIS score.
...
These show that, where compounds had *m/z* features matching to theoretical ionisation products, the approach was able to assign `r compounds_percent_assigned$standards`% and `r compounds_percent_assigned$spiked_urine`% of these chemical standards in the standards mix and spiked urine respectively. 
The *m/z* for which a chemical standard was assigned are provided in Additional file 6.
...
standards_compound_outcomes %>%
  ...
  kableExtra::kable(
    ...
    caption = 'Molecular formula assignment results frequencies for the 64 unique molecular formulas of the chemical standards in the standards mix and spiked into human urine.'
  ) %>%
  kableExtra::kable_styling(
    latex_options = "HOLD_position"
  )
...
An overview of the molecular formula assignments made across the example sample matrices is provided in Table S6 (Additional file 3).
All of the assignments for these example matrices along with any matched KEGG compound IDs are provided in Additional file 4.
...
The chromatographic separation of the C18-UHPLC-HRMS enables the correlation network to be considered within limited retention time windows and increases the likelihood that a number of correlated ionisation products will be present for a given compound.
