...
spiked_urine_search_ppm <- 6

spiked_urine_assignment_parameters <- {
  ap <- assignments::assignmentParameters('FIE')
  assignments::adducts(ap)$n <- c("[M-H]1-",
                                  "[M+Cl]1-",
                                  "[M+K-2H]1-",
                                  "[M+Cl37]1-",
                                  "[M-2H]2-",
                                  "[2M-H]1-")
  assignments::ppm(ap) <- spiked_urine_search_ppm
  assignments::adducts(ap)$p <- assignments::adducts(ap)$p[!stringr::str_detect(assignments::adducts(ap)$p,
                                                                                stringr::coll("[M+NH4]1+"))]
  assignments::isotopes(ap) <- c('13C','18O','34S','13C2')
  ap
}

spiked_urine_example_feature <- 'n80.97488' 

spiked_urine_targets <- list(
  ...
  ## download mzML files
  hrmtargets::tar_input_piggyback(spiked_urine,
                                  'spiked-urine',
                                  sample_info_file = 'spiked_urine_runinfo.csv'),
  
  ## Perform spectral binning
  hrmtargets::tar_spectral_processing(spiked_urine,
                                      type = 'FIE-HRMS',
                                      export_path = NULL),
  ...
  hrmtargets::tar_mf_assignment(spiked_urine,
                                parameters = spiked_urine_assignment_parameters,
                                export_path = NULL),
